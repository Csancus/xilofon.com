...
export default async function LandingPage({ params }: Props) {
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-white">
      {/* Logo bar */}
      <div className="border-b border-white/[0.06] py-4 px-4">
        ...
        <div className="relative max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-violet-400 mb-5">{t("price")}</p>
              <p className="text-white/60 leading-relaxed mb-8">{t("subtitle")}</p>
              ...
              <div className="grid grid-cols-3 gap-4">
                {(["clients", "time", "price"] as const).map((key) => (
                  ...
                  >
                    <div className="text-xl font-bold text-white">{t(`${key}Num`)}</div>
                    <div className="text-xs text-white/40 mt-0.5">{t(`${key}Label`)}</div>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="lg:sticky lg:top-8">
              <div className="p-7 rounded-2xl bg-white/[0.04] border border-white/[0.08]">
                <h2 className="text-xl font-bold text-white mb-1">{t("formTitle")}</h2>
                <p className="text-white/50 text-sm mb-6">{t("formSubtitle")}</p>
                <ContactForm />
              </div>
