        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
          {serviceKeys.map((key, i) => {
            ...
            return (
              <div
                key={key}
                className="p-8 rounded-2xl bg-white/[0.03] border border-white/[0.07] hover:border-violet-500/20 transition-all"
              >
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                  <div>
                    <div className="flex items-center gap-4 mb-4">
                      ...
                      <div>
                        <h2 className="text-xl font-bold text-white">{t(`${key}.title`)}</h2>
                        <span className="text-xs text-violet-400 font-medium">{t(`${key}.price`)}</span>
                      </div>
                    </div>
                    <p className="text-white/40 text-sm italic mb-4">{t(`${key}.tagline`)}</p>
                    <p className="text-white/60 text-sm leading-relaxed">{t(`${key}.desc`)}</p>
                    <Link
                      href="/landing"
